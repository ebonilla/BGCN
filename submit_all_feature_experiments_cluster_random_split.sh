...
RESULTS_DIR=$SCRATCH1DIR'/'$all_model'/random_splits' # output for all results
...
then
    ...
    str_options='--dataset='$DATASET_NAME' --adjacency='$ADJ_MATRIX' --random-seed-np='$RANDOM_SEED_NP' --random-seed-tf='$RANDOM_SEED_NP' --random-split  --add-val --add-val-seed='$SEED_VAL' --random-split-seed='$RANDOM_SPLIT_SEED' --split-sizes='$SPLIT_SIZE
